...
import com.ajinkya.linky.config.JwtAuthFilter;
...
public class AuthController {
    ...
    public ResponseEntity<LoginResponse> login(@Valid @RequestBody LoginRequest request) {
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.SET_COOKIE, accessTokenCookie(loginResponse.getAccessToken()).toString())
                .body(loginResponse);
    }
    ...
    /**
     * Lets the owner of a PRIVATE short link authenticate via plain browser navigation
     * (RedirectController), where no Authorization header is ever sent.
     */
    private ResponseCookie accessTokenCookie(String accessToken) {
        return ResponseCookie.from(JwtAuthFilter.ACCESS_TOKEN_COOKIE, accessToken)
                .httpOnly(true)
                .secure(cookieSecure)
                .sameSite("Lax")
                .path("/")
    ...
    private ResponseCookie clearedAccessTokenCookie() {
        return ResponseCookie.from(JwtAuthFilter.ACCESS_TOKEN_COOKIE, "")
                .httpOnly(true)
                .secure(cookieSecure)
                .sameSite("Lax")
                .path("/")
